...
void I2CPayload::show_payload(){
    byte i;
    byte words[I2C_VALUE_BYTE_MAX];
    int numbers[I2C_VALUE_INT_MAX];
    Serial.print("p\n");
    switch(this->get_value_type()){
        case I2C_VALUE_TYPE_BYTE:
            this->get_bytes(words);
            Serial.print("b c = "+String(this->get_command())+" v  = "+String((char*)words)+"\n");
            break;
        case I2C_VALUE_TYPE_INT:
            this->get_ints(numbers);
            Serial.print("i c = "+String(this->get_command())+"\n");
            for(i=0;i<I2C_VALUE_INT_MAX;i++){
                Serial.print("v["+String(i)+"] = "+String(numbers[i])+"\n");
            }
            break;
        default:
            Serial.print("uvt\n");
